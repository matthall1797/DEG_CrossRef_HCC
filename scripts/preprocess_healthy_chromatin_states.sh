...
echo "Combining Healthy Chromatin State beds..."
...
cat "$TMPDIR"/*_with_sample.bed > "$TMPDIR/healthy_calls.bed"

sort -k1,1V -k2,2n "$TMPDIR/healthy_calls.bed" > "$TMPDIR/healthy_calls_sorted.bed"
...
awk 'BEGIN{OFS="\t"} \
    ...
    }' "$TMPDIR/healthy_calls_sorted.bed" > "$TMPDIR/merged_healthy_calls.bed"

sort -k1,1V -k2,2n "$TMPDIR/merged_healthy_calls.bed" > final_healthy_calls.bed

echo "Done processing Healthy Chromatin States"
